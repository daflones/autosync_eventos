...
Deno.serve(async (req: Request) => {
  try {
    // ✅ CORRIGIDO: Definir valores diretamente com seus dados
    const supabaseUrl = 'https://yczwxthqfladdufmcprm.supabase.co';
    const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inljend4dGhxZmxhZGR1Zm1jcHJtIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1NjE1MTY3MywiZXhwIjoyMDcxNzI3NjczfQ.PCKLhFl1LwkYC0BV8k5YLJcbk0nQvlZBzX80zmw4bsY';
    const webhookUrl = 'https://n8n.agenciagvcompany.com.br/webhook/disparador_eventos';
    
    const supabase = createClient(supabaseUrl, supabaseKey);

    console.log('🔍 Buscando campanhas ativas...');

    // Buscar campanhas com status 'dispatching' (não incluir completed, cancelled, dispatched)
    ...
    for (const campaign of activeCampaigns){
      ...
      // DEBUG: Verificar quantos registros existem para esta campanha
      const { data: allSends, error: debugError } = await supabase
        .from('disparador_sends')
        .select('id, status')
        .eq('campaign_id', campaign.id);
      
      console.log(`🔍 DEBUG - Total de registros para campanha ${campaign.id}:`, allSends?.length || 0);
      if (allSends) {
        const statusCounts = allSends.reduce((acc, send) => {
          acc[send.status] = (acc[send.status] || 0) + 1;
          return acc;
        }, {});
        console.log(`📊 Status dos registros:`, statusCounts);
      }
      
      // Buscar próximo envio pendente (query simplificada)
      const { data: nextSends, error: sendsError } = await supabase
        .from('disparador_sends')
        .select('id, customer_id, customer_name, remotejid, message_content, campaign_id')
        .eq('campaign_id', campaign.id)
        .eq('status', 'pending')
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Verificar se atingiu 30 envios e marcar como completed
          const { data: sentCount } = await supabase
            .from('disparador_sends')
            .select('id', { count: 'exact', head: true })
            .eq('campaign_id', nextSend.campaign_id)
            .eq('status', 'sent');
          
          if (sentCount >= 30) {
            console.log(`🎯 Campanha ${nextSend.campaign_id} atingiu 30 envios - marcando como completed`);
            await supabase
              .from('disparador_campaigns')
